...
| `samenet`           | 匹配服务器任何子网段对应的 IP 地址 |
| `127.0.0.1`         | 匹配本地回源地址                   |
| `192.168.66.256/32` | 匹配单个 IP                        |
| `192.168.66.0/24`   | 匹配 IP 段                         |

::: warning :warning:`local连接类型` 通过 `Unix域套接字连接` ，没有 `ADDRESS` 选项
