...
| 路径            | 授权用户          | 授予权限                         |
| --------------- | ----------------- | -------------------------------- |
| `C:\php\74`     | `PHP74_CGI_Users` | 基本权限-`列出文件夹内容`+`读取` |
| `C:\php\84`     | `PHP84_CGI_Users` | 基本权限-`列出文件夹内容`+`读取` |
| `C:\php\config` | `PHP_CGI_Users`   | 基本权限-`列出文件夹内容`+`读取` |
| `C:\php\tools`  | `PHP_CGI_Users`   | 基本权限-`列出文件夹内容`+`读取` |
...
我们不推荐全局添加 FastCGI 模块，本次案例也是在创建站点时单独添加 FastCGI 模块，详情请阅读 [[站点应用案例-处理程序映射截图]](#handler-mapping)

## 4、站点应用案例 {#site-app-case}
...
| 用户授权                   | 目录范围                          | 授予权限                  |
| -------------------------- | --------------------------------- | ------------------------- |
| `iis_php84` 或 `iis_php74` | 项目根目录及子目录                | `基本权限-读取`           |
| `iis_php84` 或 `iis_php74` | 上传目录 `/public/static/uploads` | `基本权限-读取+写入+修改` |
